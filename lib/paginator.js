...
const sorterList = (prop = 'name', order = 'asc', list = []) => {
  ...
  //slice it to keep immutability
  return list.slice().sort((compA, compB) => {
    let a = compA;
    let b = compB;
...
/** Transducer that extends a reducer with pagination abilities
  ...
  @param behaviors (object) : pagination behavior methods available for customization :
    @param 'extractor' (function) : Optional. Function which extract part of the wrapped state where pagination occurs.
    @param 'sortor' (function) : Optional. Function used for sorting.
    @param 'filtor' (function) : Optional. Function used for filtering.
  @param initOptions (object) : Optional. Default pagination options
  @returns high order reducer with pagination
 */
export function paginated(type, reducer, { extractor = noop, sortor = sorterList, filtor = filterList } = {}, initOptions = {}) {
  ...
  return function transducer(state = initialState, action) {
      ...
      case `${type}_${PAG_RESET}`:
        {
          ...
          const newCache = sortor(options.by, options.order, filtor(options.filter, list));
      ...
      case `${type}_${PAG_FILTER}`:
        {
          ...
          const newCache = sortor(by, order, filtor(newFilter, list));
      ...
      case `${type}_${PAG_SORT}`:
        {
          ...
          const newCache = newBy === by ? reverserList(cacheList) : sortor(newBy, newOrder, filtor(filter, list));
    ...
    const newCache = sortor(by, order, filtor(filter, extractor(newBase)));
