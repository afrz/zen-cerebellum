  ...
  return function reducerCRUD(state = initialState, action) {
    ...
    switch (action.type) {
      ...
      case `${type}_CREATE_RESPONSE`:
        //action.payload is the complete object sent back by server (merge it with initial object)
        return mutate(state, {
          items: addElement(state.items,
            mutate(action.meta.genuine, action.payload)
          )
        });
